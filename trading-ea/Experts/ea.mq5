...
//|   Version: 1.0                                                   |
//|   Author: Your Name                                              |
//|   Copyright © 2024 Your Name / Your Brand                        |
//|                                                                  |
//|   Description:                                                   |
//|   Expert Advisor based on price-action.            |
//|   Executes trades automatically based on defined rules.           |
//|                                                                  |
//|   NOTE: Trading involves risk. Use with proper risk management.   |
//+------------------------------------------------------------------+

#property version     "1.00"
#property copyright   "© 2025 Covenant Monday / YourBrand"
#property link        "https://yourwebsite.com"
#property description "Price Action EA"
...
#include "Include/Strategy/IStrategy.mqh"
#include "Include/Strategy/Breakout.mqh"
#include "Include/Strategy/Reversal.mqh"
#include "Include/Strategy/CandlePattern.mqh"
...
input int BreakoutLookback = 20;     // Lookback period for breakout
input double BreakoutThreshold = 10;  // Breakout threshold in points
input int ReversalLookback = 10;     // Lookback period for trend
input double MinCandleSize = 10;     // Minimum candle size in points

input int TrendPeriod = 20;        // Period for trend detection
input int PatternPeriod = 3;       // Period for pattern detection
input double MinBodySize = 10;     // Minimum candle body size in points
...
int OnInit()
{
    ...
    ActiveStrategy = new EnhancedPatternStrategy(TrendPeriod, PatternPeriod, DefaultStopLoss, DefaultTakeProfit, MinBodySize);
...
double StopLoss(ENUM_ORDER_TYPE order_type, double open_price)
{
    ...
    return ActiveStrategy.GetStopLoss(order_type, open_price);
}
...
{
    if (!ActiveStrategy) return 0;
    return ActiveStrategy.GetTakeProfit(order_type, open_price);
}
...
void CheckEntrySignal()
{
    ...
    if (ActiveStrategy.ShouldEnterLong())
    {
        OpenBuy();
    }
    else if (ActiveStrategy.ShouldEnterShort())
    {
        OpenSell();
    }
}
...
void CheckExitSignal()
{
    ...
    if (ActiveStrategy.ShouldExitLong())
    {
        CloseAllBuy();
    }
    if (ActiveStrategy.ShouldExitShort())
    {
        CloseAllSell();
    }
}
